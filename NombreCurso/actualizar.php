...
a.button:hover {
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/prefixfree/1.0.7/prefixfree.min.js"></script>
	...
	<?php
	include ('conex2.php');
	$ras= mysql_query("select Correo,Password,NivelUS from usuarios where Correo ='".$cookie_value."'");
	
	if($ext = mysql_fetch_array($ras)){
	if ($ext['NivelUS'] == 1 ){
	?>
...
if(isset($_POST['submit'])){
	...
	if ( $objCliente->actualizar(array($Nombre),$nc_id) == true){
		...
		var respuesta=confirm("El nombre fue actualizado, Desea regresar al menu inicial") 
		if (respuesta==true)
...
<form id="frmClienteActualizar" name="frmClienteActualizar" method="post" action="actualizar.php" onSubmit="ActualizarDatos(); return false">
    ...
    <div align="center">
        <input type="hidden" name="nc_id" id="nc_id" value="<?php echo $nc['Id']?>" />
  </div>
  <p align="center">
  <hr>
  <label id="icon" for="name"><i class="icon-user"></i></label><label> Nombre<br />
    <br>
    <input class="text" type="text" name="Nombre" id="Nombre" value="<?php echo $nc['Nombre']?>" />
    </label>
  </p>
  <p align="center">&nbsp;</p>
  <p align="left">
