...
if(isset($_POST['submit'])){
	...
	if ( $objCliente->actualizar(array($IdDependencia,$Nomb),$ar_id) == true){
...
<form id="frmClienteActualizar" name="frmClienteActualizar" method="post" action="actualizar.php" onSubmit="ActualizarDatos(); return false">
  ...
  <div align="center">
    <input type="hidden" name="ar_id" id="ar_id" value="<?php echo $info['Id']?>" />
  </div>
    <p align="center">
    <hr>
  ...
  <label>Dependencia
	    <select name="IdDependencia" id="IdDependencia">
			...
			$valorSeleccionado = $info['IdDependencia'];
			while($arreglo = mysql_fetch_array($query)){
			?>
			<option <?php if ($arreglo['Id']==$info['IdDependencia']){ echo "selected"; }?> value="<?php echo $arreglo['Id']?>"><?php echo $arreglo['nombre']?>
			<?php
			}
			?>		
			</select>
  ...
  <label>Nombre
  <input class="text" type="text" name="Nomb" id="Nomb" value="<?php echo $info['Nomb']?>" />
  </label>
  </p>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
