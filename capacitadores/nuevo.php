...
if(isset($_POST['submit'])){
	...
	$sql="INSERT INTO personas (Nombre,CURP,FechaNac,RFC,Sexo,Estatus,Tipo) VALUES ('".$Nombre."','".$CURP."','".$FechaNac."','".$RFC."','".$Sexo."','Activo','Capacitador')";
	 $sql2 = "SELECT Id from personas order by Id desc limit 1";

		$rs = mysqli_query($con,$sql2);
		
		$rog = mysqli_fetch_array($rs);
	...
	$Numero = htmlspecialchars(trim($_POST['Numero']));
			...
			$sql1="INSERT INTO capacitadores (IDPersona,Curriculum,Empresa) VALUES ('".$rug."','".$Curriculum."','".$Empresa."')";
			$sql3="INSERT INTO telefonos (id_TipoPersona,TipoDpersona,TipoTelefono,Numero) VALUES ('".$rug."','Capacitador','".$TipoTelefono."','".Numero."')";
	...
	if ( $query_update){
		...
		var respuesta=confirm("El capacitador fue dado de alta, Desea regresar al menu inicial") 
		if (respuesta==true)
		 window.location='index.php';
		else
		window.location='nuevo.php'; </script>
...
<form id="frmClienteNuevo" name="frmClienteNuevo" method="post" action="nuevo.php" onSubmit="GrabarDatos(); return false">
 ...
 <label> Numero</label><label id="icon" for="name"><i class="icon-user"></i></label>
  <input class="text" type="text" name="Numero" id="Numero" />
   ...
   <hr>
 <label> Curriculum</label><label id="icon" for="name"><i class="glyphicon glyphicon-folder-open"></i></label>
  <input class="text" type="text" name="Curriculum" id="Curriculum" />
  </p>
   <hr>
   <label>Empresa </label><label id="icon" for="name"><i class="glyphicon glyphicon-info-sign"></i></label>
  <input class="text" type="text" name="Empresa" id="Empresa" />
  </p>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
  <p>
    ...
    <label></label>
    <input type="button" class="cancelar" name="cancelar" id="cancelar" value="Cancelar" onClick="window.location='usuarios.php'" />
  </p>
</form>
<?php
}
